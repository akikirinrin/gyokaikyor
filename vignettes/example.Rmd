...
sum_jul_nov <- function(dat, prefec, fishery = unique(dat$Fishuery)) {
  dat %>%
...
豊後水道南部東側では、1歳魚（`r year()`年級群）が漁獲の主体で、0歳魚（2020年級群）も混じる。1歳魚以上の当海域における2019年の漁獲量がほぼ前年並であったことから、今期の来遊量は前年並と予測される。
宿毛湾・土佐湾では、0歳魚（2020年級群）と1歳魚（`r year()`年級群）が主体となる。宿毛湾まき網の前年下半期漁獲量と上半期漁獲量には正の関係が認められる。2019年7月～12月は前年を下回っているが、2019年1月～6月は予測を大幅に下回ったことから、今期の来遊量は前年を上回ると予測される。
...
熊野灘では、まき網、定置網の7月～11月の漁況から、主体となる1歳魚（`r year()`年級群）は前年を下回ると考えられる。期後半に定置網で漁獲される0歳魚（2020年級群）については海況の経過が前年に類似すると予測されることから前年並と予測される。2歳以上は少ない。以上から、今期の来遊量は前年を下回ると予測される。
...
```{r, echo = FALSE}
```
房総～鹿島灘海域における北部まき網の`r year()`年7月～11月の漁獲量は`r editme("1,372")`トンとなり、前年同期`r editme("47")`トンを大きく上回った。仙台湾～三陸海域では、宮城県の`r year()`年7月～11月の漁獲量は`r sum_jul_nov(maaji, "宮城")`トン、岩手県の2019年7月～11月の定置網による漁獲量は`r sum_jul_nov(maaji, "岩手", "定置網")`164`r editme("あってる？")`トンで、それぞれ前年を上回った。青森県では2019年7月～11月に八戸の漁獲量は`r editme(0.2)`トン、平舘の定置網による漁獲量は`r editme(88)`トンであった。
...
plot_rawdata <-
  ...
  labs(title = "加入量指標値 (`r year()`年まで)", x = "", y = "加入量指標値 (対数値)")
